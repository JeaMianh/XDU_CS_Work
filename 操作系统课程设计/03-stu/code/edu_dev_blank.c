...
int kthread_handler(void *data)
{
	...
	printk("kthread handler input: %lld\n", value);
	...
	printk("kthread handler result: %lld\n", atomic64_read(&thread_data_ptr->user_data_ptr->data));

	wake_up(&thread_data_ptr->user_data_ptr->wq); 
...
static int edu_dev_open(struct inode *inode, struct file *filp)
{
	...
	init_waitqueue_head(&user_data_ptr->wq);
...
long edu_dev_unlocked_ioctl(struct file *pfilp_t, unsigned int cmd, unsigned long arg)
{
	...
	{
		case 0:
			thread_data = (struct thread_data *)kmalloc(sizeof(struct thread_data), GFP_KERNEL);
			if (!thread_data)
			{
				printk(KERN_ERR "thread data kmalloc fail!\n");
				return -ENOMEM;
			}

			thread_data->input_data = arg;
			thread_data->user_data_ptr = user_data_ptr;

			printk("ioctl input: %d\n", thread_data->input_data);

			thread = kthread_run(kthread_handler, thread_data, "edu_factorial_thread");
			if (IS_ERR(thread))
			{
		...
		case 1:
			...
			wait_event_interruptible(user_data_ptr->wq, atomic64_read(&user_data_ptr->data) != 0);

			if (ret) return ret;

			result = atomic64_read(&user_data_ptr->data);

			printk("ioctl result: %lld\n", result);

			printk("copy to user arg: %lu, result: %llu\n", arg, result);
			if (copy_to_user((unsigned long *)arg, &result, sizeof(result)))
			{
